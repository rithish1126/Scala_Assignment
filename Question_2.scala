...
class Helperclass(arr:Array[player]){//class that helps to impliment different function related to the question asked, and takes an Array of players as primary constructor
  ...
  def top5Runs(): Unit = {
    ...
    if (arr.length < 5) {
      println("Less than 5 players please enter more players to get the top 5 players with highest runs scored")
      return
    }
  ...
  def top5Wickets(): Unit = {
    ...
    if (arr.length<5)
      {
        println("Less than 5 players please enter more players to get the top 5 players with highest wickets taken")
        return
      }
